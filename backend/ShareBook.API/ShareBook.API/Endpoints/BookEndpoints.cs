...
public static class BookEndpoints
{
    public static void MapBookEndpoints(this WebApplication app)
    {
        ...
        bookGroup.MapPost("/", async (IBookRepository bookRepository, BookDto book) =>
        {
        ...
        bookGroup.MapGet("/{id:guid}", async (IBookRepository bookRepository, Guid id) =>
        {
            var book = await bookRepository.GetByIdAsync(id);
            return book is not null ? Results.Ok(book) : Results.NotFound();
        });

        bookGroup.MapGet("/", async (IBookRepository bookRepository, int pageNumber = 1, int pageSize = 10) =>
        {
            var books = await bookRepository.GetAllAsync(pageNumber, pageSize);
            return Results.Ok(books);
        });

        bookGroup.MapPut("/{id:guid}", async (IBookRepository bookRepository, Guid id, BookDto updatedBook) =>
        {
        ...
        bookGroup.MapDelete("/{id:guid}", async (IBookRepository bookRepository, Guid id) =>
        {
